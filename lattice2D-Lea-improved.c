...
// --- For density averaging after 10000 ---
double AccumDensity[Lx] = {0};
...
void WriteConfig(long int index, bool track_occupancy, bool track_density, bool track_flux)
{
    ...
    if (!DensityAveraging) {
        ...
        filter_density = (index == TotalTime || index == -1 || index == 0 || (index <= DensityAveragingStart && (index % 1000 == 0)) || (index <= DensityAveragingStart && (SaveInterval > 0 && index % SaveInterval == 0)));
    }
    if (track_density && filter_density && (!DensityAveraging || index <= DensityAveragingStart)) {
    ...
    // Write averaged density if enabled
    // Write final averaged density if enabled
    if (index == TotalTime && DensityAveraging) {
        char filename[256];
        FILE *f;
        sprintf(filename, "%s/Density_avg_start%ld.dat", RunName, DensityAveragingStart);
        f = fopen(filename, "w");
        if (!f) {
            fprintf(stderr, "[ERROR] Could not open %s for writing final averaged density\n", filename);
        } else {
            for (int i = 0; i < Lx; i++) {
                fprintf(f, "%.6f ", AccumDensity[i] / (AccumDensitySteps * Ly));
            }
            fclose(f);
            fprintf(stderr, "Final averaged density written to %s (steps: %ld)\n", filename, AccumDensitySteps);
        }
    }
    ...
    if (!DensityAveraging) {
        ...
        // If density averaging is ON, only write up to and including DensityAveragingStart
        filter_flux = (index == TotalTime || index == -1 || index == 0 || index == DensityAveragingStart);
    }
...
typedef struct {
    ...
    bool density_averaging; // Enable density averaging after a given step
    long int density_averaging_start; // Step at which averaging starts
} SimulationParams;
...
void InitializeDefaultParams(SimulationParams *params) {
    ...
    params->density_averaging = false; // Default: off
    params->density_averaging_start = 10000; // Default: start at step 10000
}
...
int ParseArguments(int argc, char **argv, SimulationParams *params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "--density-averaging") == 0) {
            params->density_averaging = true;
...
int main(int argc, char **argv)
{
    ...
    DensityAveraging = params.density_averaging;
    DensityAveragingStart = params.density_averaging_start;
