...
void WriteConfig(long int index)
{
	...
	sprintf(filename,"%s/Occupancy_%05ld.dat",RunName,index);
	f=fopen(filename,"w");
	for(j=0;j<Ly;j++)
	{
		for(i=0;i<Lx;i++)
	...
	sprintf(filename,"%s/Config_%05ld.dat",RunName,index);
	f=fopen(filename,"w");
	for(n=0;n<NParticles;n++)
...
int main(int argc, char **argv)
{
	...
	if(argc != 5)
	{
		fprintf(stderr,"Usage: %s <density> <tumbling_rate> <total_time> <run_name>\n", argv[0]);
		return 1;
	}
