...
void InitializeMoveProbMap(void) {
    ...
    double area_curve = integrate_curve(uneven_sin_function, 0, M_PI, 1000);
    fprintf(stderr, "Initializing movement probability map (area_curve = %.6f)...\n", area_curve);
    
    for (int x = 0; x < Lx; x++) {
        for (int y = 0; y < Ly; y++) {
            double scale_x = ((double)x / Lx) * M_PI;
			fprintf(stderr, "Calculating MoveProbMap[%d][%d] with scale_x = %.6f\n", x, y, scale_x);
            double raw_value = uneven_sin_function(scale_x);
			fprintf(stderr, "Raw value for MoveProbMap[%d][%d] = %.6f\n", x, y, raw_value);
            MoveProbMap[x][y] = raw_value / area_curve;
            
            // Debug: print first few values
            if (x < 3 && y < 3) {
                fprintf(stderr, "MoveProbMap[%d][%d] = %.6f (raw=%.6f)\n", x, y, MoveProbMap[x][y], raw_value);
            }
        }
    }
